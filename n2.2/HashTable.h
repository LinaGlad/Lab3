...
public:
  ...
  void serializeBinary(std::ostream &out) const {
    ...
    for (size_t i = 0; i < capacity; ++i) {
      if (table[i].state == EntryState::OCCUPIED) {
        size_t key_len = table[i].key.size();
        out.write(reinterpret_cast<const char *>(&key_len), sizeof(key_len));
        out.write(table[i].key.c_str(), key_len);
        out.write(reinterpret_cast<const char *>(&table[i].value),
                  sizeof(table[i].value));
      }
    }
  }
  ...
  void deserializeBinary(std::istream &in) {
    ...
    if (!in.read(reinterpret_cast<char *>(&new_capacity), sizeof(new_capacity)))
      return;
